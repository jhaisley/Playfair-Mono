...
class Git:
    def __getattr__(self, name):
        def wrapper(*args, **kwargs):
            # Convert python method names to git subcommand format if needed,
            # though here they map directly (remote, add, status, commit, push)
            cmd = ["git", name] + list(args)
...
print(copyright)
with open("OFL.txt", "w", encoding="utf-8") as fh:
    fh.write(copyright)
    fh.write("\n".join(ofl[1:]))
